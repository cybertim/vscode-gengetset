...
export const modeId: vscode.DocumentSelector = 'typescript';
...
export class SuggestImport implements vscode.CompletionItemProvider {
  ...
  private exposeCache: IExpose[] = null;
  ...
  provideCompletionItems(document: vscode.TextDocument, position: vscode.Position, token: vscode.CancellationToken): Promise<vscode.CompletionItem[]> {
    return new Promise<vscode.CompletionItem[]>((resolve, reject) => {
      ...
      if (provideCompletion) {
        while (!this.ready) { } // ugh, should change...
        if (this.ready) resolve(this.toItemArray());
      } else {
        resolve([]);
      }
    });
    ...
    for (let i = 0; i < lineCount; i++) {
      ...
      for (let j = 0; j < matches.length; j++) {
        if (matches[j].indexOf('\'') == -1 && matches[j].indexOf('\"') == -1 && this.commonList.indexOf(matches[j]) == -1) {
          ...
          const cword: string = splitted.length > 0 ? splitted[0] : matches[j];
              ...
              if ((!this.exposeCache[k].dict && this.exposeCache[k].exported.indexOf(cword) !== -1) ||
                ...
                for (let l = 0; l < list.length; l++) {
                  if (list[l].name === this.exposeCache[k].name) {
                    if (list[l].exported.indexOf(cword) == -1) list[l].exported.push(cword);
                    found = true;
                    break;
                  }
                }
  ...
  private toItemArray(): vscode.CompletionItem[] {
    ...
    for (let i = 0; i < this.exposeCache.length; i++) {
      for (let j = 0; j < this.exposeCache[i].exported.length; j++) {
        let item = new vscode.CompletionItem(this.exposeCache[i].exported[j]);
        item.detail =
